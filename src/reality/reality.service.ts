...
export class RealityService {
  ...
  async performRitual(ritualDto: PerformRitualDto, authUser?: AuthUser) {
    ...
    const ritual = await this.ritualsService.performRitual(payload, authUser);
    const completedImmediately = ritual.moderationStatus === "completed";

    return {
      success: true,
      message: completedImmediately
        ? `Ритуал "${ritualDto.ritualType}" выполнен`
        : `Ритуал "${ritualDto.ritualType}" отправлен на модерацию`,
      data: ritual,
      effect: completedImmediately
        ? "Обряд выполнен сразу по правам администратора."
        : "Обряд зарегистрирован и передан модератору.",
      timestamp: new Date().toISOString(),
    };
  }
  ...
  async registerUser(registerUserDto: RegisterUserDto) {
    if (!this.databaseService.isAvailable()) {
      throw new ServiceUnavailableException(
        "База данных недоступна. Регистрация временно невозможна"
      );
    }
    ...
    return {
      success: true,
      message:
        "Пользователь зарегистрирован. Подтвердите email по ссылке из письма.",
      data: {
        userId,
        email: normalizedEmail,
        emailConfirmed: false,
  ...
  async confirmEmail(confirmEmailDto: ConfirmEmailDto) {
    if (!this.databaseService.isAvailable()) {
      throw new ServiceUnavailableException(
        "База данных недоступна. Подтверждение email временно невозможно"
      );
    }

    const tokenData = await this.databaseService.getEmailConfirmationToken(
  ...
  async login(loginDto: LoginDto) {
    if (!this.databaseService.isAvailable()) {
      throw new ServiceUnavailableException(
        "База данных недоступна. Вход временно невозможен"
      );
    }
  ...
  async getSupportTickets(userId: string) {
    ...
    const tickets = await this.databaseService.getSupportTicketsByUserId(
      userId
    );

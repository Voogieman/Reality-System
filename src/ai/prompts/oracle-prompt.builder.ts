import {
  SLAVIC_GODS,
  type SlavicGodDefinition,
} from "../../gods/slavic-gods.constants";
import type { OracleContext } from "../god-oracle.service";
import {
  DEFAULT_ORACLE_PROFILE,
  GOD_ORACLE_PROFILES,
} from "./god-oracle-profiles";
...
export function buildOraclePrompt(context: OracleContext): BuiltOraclePrompt {
  ...
  const profile =
    GOD_ORACLE_PROFILES[context.godName] ?? DEFAULT_ORACLE_PROFILE;
  ...
  return {
    ...
    temperature:
      profile.temperature ?? DEFAULT_ORACLE_PROFILE.temperature ?? 0.85,
  };
}
...
function buildPersonaLayer(
  ...
  const loreBlock =
    profile.lore.length > 0 ? profile.lore.join("\n") : god.description;
...
function buildUserLayer(
  ...
  if (context.offeringType != null) {
    ...
    lines.push(
      "Подношение не принесено — ответь как на чистый вопрос духа, без полного ритуала."
    );
  }
...
function timeOfDayHint(godName: string): string {
  ...
  if (hour >= 17 && hour < 21) {
    ...
    if (godName === "Ягиня") {
      return "ВРЕМЯ: сумерки на опушке — туман встаёт, порог открыт; говори вещим лесным шёпотом.";
    }
  ...
  if (hour >= 21 || hour < 5) {
    ...
    if (godName === "Ягиня") {
      return "ВРЕМЯ: ночь и лунный серп — изба видна в тумане; говори из лесной тишины.";
    }

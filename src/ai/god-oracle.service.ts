...
export class GodOracleService {
  ...
  async speak(
  ...
  ): Promise<{ prophecy: string; model: string; messageId: string }> {
    const god = SLAVIC_GODS[context.godName];
    if (!god) {
      throw new NotFoundException(
        `Бог ${context.godName} не найден в пантеоне`
      );
    }

    const { systemMessages, userMessage, temperature } =
    ...
    const messageId = `oracle_${Date.now()}_${Math.floor(
      Math.random() * 100000
    )}`;
    ...
    if (this.databaseService.isAvailable()) {
      const offering =
        ...
        context.purity !== undefined ||
        context.significance !== undefined
          ? {
